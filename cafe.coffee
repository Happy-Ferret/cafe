...
argv                  = require('optimist').boolean('wfreestanding').argv
fs                    = require 'fs'
...
Available options are:
	...
	\x1b[1;32m--freestanding\x1b[0m   Compile without any special runtime support.
	\x1b[1;32m-wfreestanding\x1b[0m  Suppress the warning generated by --freestanding
"""
...
if argv?._[0]?
	...
	inp = argv.mini
else if argv.freestanding? and typeof argv.freestanding is 'string'
	inp = argv.freestanding
else
	inp = '-'
...
if inp is '/dev/stdin' or inp is '-'
	...
	if fs.existsSync inp
		fs.readFile inp, {encoding: 'utf-8'}, (err, data) ->
			...
			if argv.mini?
				data = ";;@import minilib\n#{data}"
			else if argv.freestanding?
				if !argv.wfreestanding?
					console.log "\x1b[1;33mwarning:\x1b[0m compiling without special runtime support. Operators, '() notation, and (a . b) notation will \x1b[1;37mnot\x1b[0m work."
					console.log "         for operators, implement a function #^X where X is the operator."
					console.log "         for list notation, implement a function (list ...), "
					console.log "         for dotted-pair notation, implement a function (cons a b)."
					console.log "         this message can be suppressed with -wfreestanding."
			else
				data = ";;@import prelude\n#{data}"

			if argv.optimize is false
				optimize = (x) -> x
